...
本文旨在深入探讨在这两个框架中启用和配置 Function Call 的关键参数——`--tool-call-parser`，并提供实践指导。

## vLLM 中的 Tool Calling

vLLM 利用 `--tool-call-parser` 参数，根据所选模型，自动对工具调用请求进行格式化并解析模型响应。正确选择解析器是保证工具调用功能正常运行的前提。
...
启动 vLLM 服务时，通过 `--tool-call-parser` 参数为您的模型指定合适的解析器。
...
SGLang 同样通过 `--tool-call-parser` 参数支持工具调用。尽管其官方文档目前不如 vLLM 详尽，我们仍可依据命名约定和社区共识进行有效配置。
...
精确配置 `--tool-call-parser` 是在 vLLM 和 SGLang 中成功实现 Function Calling 的关键。vLLM 提供了清晰的解析器与模型映射，便于快速集成。SGLang 虽然文档尚在完善，但其规范的命名也为开发者提供了有效指引。为保证功能稳定与最优性能，请务必根据您选用的模型，选择最匹配的解析器。
